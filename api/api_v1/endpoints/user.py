...
from schemas.user import UserOnly, UserCreate, UserInDBBase, AllUserWithDoc 
...
def fetch_all_users(
    *,
    ...
    """
    users = crud.user.get_multi_with_document(db=db)
...
def fetch_all_users(
    *,
    ...
    Fetch users by id
    """
    user = crud.user.get_by_id_with_doc(db=db,id=user_id)
    return user
